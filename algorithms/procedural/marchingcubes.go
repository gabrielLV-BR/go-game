...
func MarchingCubes(grid structs.Grid3D[float32], isolevel float32) MeshBuilder {
	...
	for x := 0; x < grid.Dimensions.X-1; x++ {
		for y := 0; y < grid.Dimensions.Y-1; y++ {
			for z := 0; z < grid.Dimensions.Z-1; z++ {
				...
				for _, triangle := range triangles {
					meshBuilder.AddTriangle(triangle.A, triangle.B, triangle.C)
				}
			}
		}
	}
...
func polygonize(cell GridCell, isolevel float32) []structs.Triangle {
	...
	vertices := [12]mgl32.Vec3{}

	config := triangleTable[state]

	// generate intersection vertices
	for i := 0; i < 12; i++ {
		if (edgeMask & (1 << i)) != 0 {
			index1 := edgeVertexIndices[i][0]
	...
	triangles := make([]structs.Triangle, 0, 5)

	// link edges
	for i := 0; i < len(config); i += 3 {
		v1 := vertices[config[i]]
		v2 := vertices[config[i+1]]
		v3 := vertices[config[i+2]]

		triangles = append(triangles, structs.Triangle{A: v1, B: v2, C: v3})
	}
...
// For each MC case, a list of triangles, specified as triples of edge indices, terminated by
var triangleTable [][]int = [][]int{
	{},
	{0, 3, 8},
	{0, 9, 1},
	{3, 8, 1, 1, 8, 9},
