...
// TODO just allow the use of floats for the grid
// simplifies a lot
...
func vec3ElementMult(a, b mgl32.Vec3) mgl32.Vec3 {
	...
	for x := 0; x < grid.Dimensions.X-1; x++ {
		for y := 0; y < grid.Dimensions.Y-1; y++ {
			for z := 0; z < grid.Dimensions.Z-1; z++ {
				...
				trianglePoints := make([]mgl32.Vec3, len(config)-1)
				uniqueTris := make(map[mgl32.Vec3]bool)

				for _, edgeIndex := range config {
					if edgeIndex == -1 {
						break
					}

					if (edgeIndex+1)%3 == 0 {
						for k := range uniqueTris {
							trianglePoints = append(trianglePoints, k)
						}

						clear(uniqueTris)
					}

					//TODO interpolate vertices to achieve smoother effects
					...
					v0 := vec3ElementMult(mgl32.Vec3{x, y, z}.Add(cases[edge0]), dimensions)
					v1 := vec3ElementMult(mgl32.Vec3{x, y, z}.Add(cases[edge1]), dimensions)
